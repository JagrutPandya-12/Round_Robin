    ...
    // Press Enter inside arrival field to add process
  parrival.addEventListener('keydown', (event) => {
    if (event.key === 'Enter') {
  ...
  const arrival = Math.max(0, Number(parrival.value) || 0);
  const id = Date.now()+Math.random();
  const color = randColor(processes.length);
  processes.push({id,name,burst,remaining:burst,arrival,color});
  
  pname.value=''; pburst.value=''; parrival.value='';
  renderProcessList();
  ...
  resetAnimator();
   ...
   // Random Process Generator (with random arrival times)
  randomBtn.addEventListener('click', () => {
    processes = [];
    for (let i = 0; i < 4; i++) {
      const b = Math.floor(Math.random() * 8) + 2;  // random burst between 2–10
      const a = Math.floor(Math.random() * 6);       // random arrival between 0–5
      processes.push({
        id: i,
        name: 'P' + (i + 1),
        burst: b,
  ...
  // Sort processes by arrival time (for cleaner visualization)
  ...
  trace.forEach(seg=>{
    completion[seg.pid] = (completion[seg.pid]||0) < (seg.start+seg.duration)
      ? (seg.start+seg.duration) : completion[seg.pid];
  });
  ...
  let txt = '';
  trace.forEach((s,i)=>{
    txt += `${i+1}. ${s.name} — start: ${s.start}, dur: ${s.duration}\n`;
  });
